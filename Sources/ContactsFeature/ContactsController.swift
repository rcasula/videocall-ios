...
extension ContactsController {
    ...
    @IBAction func didTapAddContact(sender: Any) {
        ...
        self.present(
            UINavigationController(rootViewController: controller), animated: true, completion: nil)
    }
...
extension ContactsController: NewContactControllerDelegate {
    ...
    func newContactController(_ controller: NewContactController, didSave contact: Contact) {
        ...
        _view.tableView.insertRows(
            at: [.init(row: contacts.count - 1, section: 0)], with: .automatic)
        loadContacts()
    }
}
